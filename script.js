  ...
  try {
    ...
    agendadosRaw.forEach(row => {
      ...
      Object.keys(row).forEach(col => {
        ...
        let shortMonth = null; let extractedYear = null;
        const yearMatch = col.match(/(\d{4})/);
        if (yearMatch) extractedYear = yearMatch[1].slice(-2);
        for (const [full, short] of Object.entries(monthMap)) { if (colLower === full || colLower === short || colLower.includes(full) || colLower.includes(short)) { shortMonth = short; break; } }
        if (!shortMonth) return;
        const value = parseNumberBR(row[col]);
        if (value <= 0) return;
        const yearToUse = extractedYear || currentYearShort;
        if (!yearToUse) return;
        dadosAgendados.push({ estabelecimento: estabelecimento || "Não informado", especialidade, mes: `${shortMonth}/${yearToUse}`, agendados: value });
      });
    });
...
function renderFila(filteredFila) {
  ...
  const trendLabels = [...periods]; const trendDataReal = [...values]; const last3 = values.slice(-3);
  let growth = 0; if (last3.length >= 2) growth = (last3[last3.length - 1] - last3[0]) / (last3.length - 1);
  const lastValue = values[values.length - 1] || 0; const projectionData = Array(periods.length).fill(null);
...
function renderFilaRetroativa() {
  ...
  const filaRetroativaSubgrupo = aggregateBy(dadosFilaRetroativa, d => d.subgrupo, d => d.fila);
  
  renderPercentageTotalTable("tableFilaRetroativaEspecialidadeBody", filaRetroativaEspecialidade, "#8b5cf6");
  renderPercentageTotalTable("tableFilaRetroativaProcedimentoBody", filaRetroativaProcedimento, "#a855f7");
  
  const complexArr = [...filaRetroativaComplexidade.entries()].sort((a, b) => b[1] - a[1]);
  makeDoughnutChartWithPercentages("cFilaRetroativaComplexidade", complexArr.map(([k]) => truncateLabel(k || "Sem Dados", 28)), complexArr.map(([,v]) => v), ["#8b5cf6", "#ec4899", "#10b981", "#d97706", "#dc2626", "#3b82f6", "#059669"]);
  
  const subgrupoArr = [...filaRetroativaSubgrupo.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
  makeDoughnutChartWithPercentages("cFilaRetroativaSubgrupo", subgrupoArr.map(([k]) => truncateLabel(k || "Sem Dados", 30)), subgrupoArr.map(([,v]) => v), ["#8b5cf6", "#c084fc", "#a855f7", "#9333ea", "#7e22ce", "#6b21a5", "#581c87", "#4c1d95", "#3b0764", "#2e1065"]);
  
  const tbodyCompleta = el("tableFilaRetroativaCompletaBody");
  if (tbodyCompleta) {
    const sortedData = [...dadosFilaRetroativa].sort((a, b) => b.fila - a.fila);
